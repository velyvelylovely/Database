...
`root`계정으로 접속하여 `DROP USER 'testuser'@'localhost';`를 실행한 후 MySQL의 모든 사용자의 호스트, 사용자 이름, 비밀번호 정보를 조회해 보면 `testuser`는 더 이상 조회되지 않습니다.

```
C:\dev\mariadb-10.6.15\bin>mysql -u root -p
Enter password: ********
Welcome to the MariaDB monitor.  Commands end with ; or \g.
